...
//=== Controller
export const blogController = ng.controller("BlogController", [
  "$scope",
  "$sce",
  "route",
  "model",
  "$location",
  "$rootScope",
  ($scope: BlogControllerScope, $sce, route, model, $location, $rootScope) => {
    ...
    route({
      ...
      print: function (params) {
        ...
        if (params.comments === "true") $scope.showComments = true;
        $scope.blog.open(
          function () {
            $scope.blog.posts.syncAllPosts(function () {
              if (!postId) {
                $scope.blog.posts.all = $scope.blog.posts.all.filter(
                  (p) => p.state == "PUBLISHED"
                );
              } else {
                $scope.blog.posts.all = $scope.blog.posts.all.filter(
                ...
                if (--countDown <= 0) {
                  setTimeout(() => {
                    ...
                    const isComplete = (img) => {
                      return (
                        img.complete || (img.context && img.context.complete)
                      );
                    };
                    $scope.printed = false;
                    const it = setInterval(() => {
                      const pending = imgs.filter(
                        (img) => !(img as any).error && !isComplete(img)
                      );
                      if (pending.length == 0) {
                        clearInterval(it);
                        if (!$scope.printed) {
              ...
              $scope.blog.posts.forEach(async function (post) {
                if (params.comments) await post.comments.sync();
                post.open(function () {
                  onFinish();
                });
              });
      ...
      editBlog: function (params) {
        ...
        const callback = () => {
          if ($scope.blog) {
            ...
            $scope.blog =
              new Behaviours.applicationsBehaviours.blog.model.Blog();
            template.open("main", "edit-blog");
          }
        };
    ...
    $scope.resetSearching = function () {
      ...
      if (!$scope.display.postSearch || $scope.display.postSearch === "")
        $scope.launchSearchingPost("");
    };
    ...
    $scope.postComment = function (comment, post) {
      ...
      $scope.comment =
        new Behaviours.applicationsBehaviours.blog.model.Comment();
    ...
    $scope.orderBlogs = function (blog) {
      let discriminator = 0;
      if (blog.myRights.editBlog) discriminator = 2;
      else if (blog.myRights.createPost) discriminator = 1;
    ...
    $scope.print = function (printComments) {
      if (
        $scope.blog.posts.some((post) => post.comments.all.length > 0) &&
        !$scope.display.showPrintComments
      ) {
        ...
        $scope.display.showPrintComments = false;
        window.open(
          `/blog/print/blog#/print/${$scope.blog._id}?comments=${printComments}`,
          "_blank"
        );
      }
    };
    $scope.printPost = function (post, printComments) {
      ...
      if (
        $scope.postToPrint.comments.all.length > 0 &&
        !$scope.display.showPrintComments
      ) {
        ...
        $scope.display.showPrintComments = false;
        window.open(
          `/blog/print/blog#/print/${$scope.blog._id}/post/${$scope.postToPrint._id}?comments=${printComments}`,
          "_blank"
        );
      }
    };
    $scope.isCloseConfirmLoaded = function () {
      return (
        angular.element("share-panel .share").scope() &&
        angular.element("share-panel .share").scope().display
    ...
    const trashOneBlog = async (blog: BlogModel) => {
      ...
      if ($scope.currentFolder) await $scope.currentFolder.sync();
    ...
    $scope.moveOneBlog = async (blog: BlogModel) => {
      const _blog =
        Folders.root.findRessource(blog._id) ||
        new Blog({ _id: blog._id } as any);
    ...
    $scope.currentVisibility = () => {
      return $scope.blog && $scope.blog.visibility
        ? $scope.blog.visibility.toLowerCase()
        : "";
    };
