  ...
  Post: function (data) {
    ...
    this.collection(Behaviours.applicationsBehaviours.blog.model.Comment, {
      ...
      remove: async function (comment) {
        await http.delete(
          "/blog/comment/" + that.blogId + "/" + that._id + "/" + comment.id
        );
  ...
  register: function () {
    ...
    this.Post.prototype.create = function (callback, blog, state) {
      oldHttp()
        ...
        .done(
          function (data) {
            let post = new Behaviours.applicationsBehaviours.blog.model.Post(
              data
            );
    ...
    this.Post.prototype.comment = function (comment) {
      return new Promise<void>((resolve, reject) => {
        oldHttp()
          .postJson("/blog/comment/" + this.blogId + "/" + this._id, comment)
          .done(
            function () {
              this.comments.sync();
              resolve();
    ...
    this.Post.prototype.updateComment = function (comment) {
      oldHttp()
        .putJson(
          "/blog/comment/" + this.blogId + "/" + this._id + "/" + comment.id,
          comment
        )
        .done(
          function () {
    ...
    articles: {
      ...
      controller: {
        init: function () {
          ...
          let blog = new Behaviours.applicationsBehaviours.blog.model.Blog({
            _id: this.source._id,
          });
          this.newPost =
            new Behaviours.applicationsBehaviours.blog.model.Post();
          blog.open(
            function () {
          ...
          this.blog.save(
            function () {
              ...
              let post = {
                state: "SUBMITTED",
                content:
                  '<p>Voici le premier billet publié sur votre site !</p><p>Vous pouvez créer de nouveaux billets (si vous êtes contributeur) en cliquant sur le bouton "Ajouter un billet" ' +
        ...
        slidePost: function (post) {
          ...
          post.open(function () {
            scope.blog.posts.forEach(function (p) {
              if (post._id === p._id) p.slided = true;
              else p.slided = false;
              scope.$apply();
            });
          });
